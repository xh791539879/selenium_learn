...
class TestAssess:
    ...
    def test_publish(self, kh_name, starttime, endtime, telephone, log):  # 发布考核
        ...
        try:
            ...
            log.logger.info(
                "考核名称{0}，开始时间{1}，结束时间{2}，咨询电话{3}".format(kh_name, starttime, endtime, telephone))

    @allure.story('按名称查询考核--测试用例')
    @pytest.mark.parametrize("name_select", test_select_data)  # 将数据传入测试用例
    def test_select_assess(self,name_select):
        sleep(5)
        select = LzkhPage()
        ...
        # try:
        #     text = driver.find_element(By.XPATH, "//tr[1]//td[3]")
        #     pass
